...
A box whose cloud session has gone to sleep can miss the wake-up ping even though it answers fine on the LAN. Fetcharr sends a wake command and pings three times before giving up, and the first attempt usually wakes the session, so retry the delete after a moment. If it keeps failing, check the box is visible in the official Fetch mobile app (same cloud path). If the app can't see it either, the box↔cloud link is down; a box restart usually resets it. More in [Troubleshooting](/guide/troubleshooting#delete-from-fetch-fails).
